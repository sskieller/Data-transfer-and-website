...
function totalRequestFunction() {
    ...
    myRequest.onload = function () {
        if (myRequest.status >= 200 && myRequest.status < 400) {
            setTableFunction(myRequest.responseText);
        } else {
...
function setTableFunction(data) {
    var allRows = data.split(/\r?\n|\r/);
    var table = '<table>';

    var tableLength = allRows.length - 2; // To remove the first faulty but needed lines
    if (tableLength >= 50) {
        tableLength = 50 + 1; // To account for the faulty lines withdrawn earlier
    }

    for (var singleRow = 0; singleRow < tableLength; singleRow++) {
        if (singleRow === 0) {
            table += '<thead>';
            table += '<tr>';
        } else {
